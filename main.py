# Codigo Principal Del Boot De ProsesoOTHCancel
...
from searchandupdateot import (searchOTHControl)
from closeapps import (closeCRM)
from storagefunctions import (showDesktop,make_noise,get_username_os,terminateProcess,getCurrentDateAndTime)
...
async def main():   
    ...
    try:
        ...
        actividad_rpa = input("1. Iniciar proceso cancelacion OTH planear con cliente \n2. No se realizara actividad\n")
        if actividad_rpa == "1":
            actividad_rpa_selected = 'Procese cancelacion OTH Planear Con Cliente'    
        else:
        ...
        for row in rows:
            ...
            if row[1].value == 'COMPLETADO' or str(row[1].value).find("ERROR") != -1 or row[1].value == 'NO OTH':
                continue
